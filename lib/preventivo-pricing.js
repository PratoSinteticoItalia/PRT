...
/**
 * Applica override di prezzo/nome/descrizione (da Impostazioni → Dati tecnici
 * prodotti) sopra il listino di default. Prezzo assente/non numerico/≤0 →
 * resta il default (mai un prodotto a prezzo 0 o rotto); nome/descrizione
 * assenti o vuoti → resta il default. Pura: nessuna chiamata di rete/DB qui,
 * il chiamante passa la mappa già risolta.
 */
export function applyProductOverrides(products, overridesBySlug = {}) {
...
/**
 * Aggiunge al listino i modelli creati da zero da Impostazioni (non presenti
 * in PRODUCTS). Una entry con isCustomProduct:true diventa un prodotto reale
 * SOLO se ha nome + entrambi i prezzi validi (niente default a cui appoggiarsi
 * per un modello nuovo — mai accettata a metà) e il suo slug non collide con
 * uno slug già presente in products. Pura, nessun I/O.
 */
export function mergeCustomProducts(products, overridesBySlug = {}) {
  const existingSlugs = new Set(products.map((p) => p.slug));
  const custom = [];
  Object.entries(overridesBySlug || {}).forEach(([slug, o]) => {
    if (!o?.isCustomProduct || existingSlugs.has(slug)) return;
    const name = String(o.name || "").trim();
    const priceCliente = Number(o.priceCliente);
    const priceRivenditore = Number(o.priceRivenditore);
    if (!name) return;
    if (!Number.isFinite(priceCliente) || priceCliente <= 0) return;
    if (!Number.isFinite(priceRivenditore) || priceRivenditore <= 0) return;
    custom.push({ id: slug, slug, name, desc: String(o.desc || "").trim(), priceCliente, priceRivenditore });
  });
 ...
 * overrideItems: se presente, sostituisce il calcolo piatto sui mq con quantitativi
 *   già pronti (stesso shape {key,label,qty,unit,unitPrice,total}) — usato quando il
 *   preventivo arriva da Garden Planner, che li calcola dalla geometria reale del
 *   disegno (perimetro/giunzioni), molto più accurata della formula sui soli mq.
 *   Vedi buildPlannerMaterialItems in garden-planner-page.js, la funzione gemella
 *   che produce questi item (duplicata lì perché quel file non può fare `import`
 *   da qui — caricato via Babel Standalone, non un modulo ES).
 */
export function getMaterialBreakdown(surface, mq, quoteType = "fornitura", excludeKeys = [], overrideItems = null) {
  const excluded = new Set(excludeKeys || []);
  if (Array.isArray(overrideItems)) {
    const marked = overrideItems.map((item) => ({ ...item, excluded: excluded.has(item.key) }));
    const total = marked.reduce((sum, item) => sum + (item.excluded ? 0 : Number(item.total) || 0), 0);
 ...
 *   materials: { include, discountPct, applyIva, autoTotal?, exclude? (key[]), items? (vedi getMaterialBreakdown) },
...
export function computeQuote(input = {}) {
  ...
  const autoBreakdown = getMaterialBreakdown(surface, mqNum, quoteType, materials.exclude || [], materials.items || null);
