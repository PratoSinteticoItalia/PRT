...
function drawSignaturesSection(doc) {
  ...
  ["FIRMA MITTENTE", "FIRMA TRASPORTATORE", "FIRMA DESTINATARIO"].forEach((label, index) => {
    ...
    doc.lineWidth(0.6).strokeColor(COLOR_BORDER).rect(x, doc.y, colW, 40).stroke();
    doc.font("Helvetica").fontSize(8).fillColor(COLOR_MUTED).text(label, x, doc.y + 44, { width: colW });
  });
}
