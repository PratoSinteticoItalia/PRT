...
function drawHeader(doc, order, logoBuffer) {
  ...
  doc.font("Helvetica").fontSize(9).fillColor(COLOR_MUTED)
    .text(
      `${isFreeDdt ? "Documento libero" : `Ordine ${order?.orderNumber || "—"}`} · ${composeRecipientLines(order)[0]?.text || "—"}`,
      MARGIN + 10,
      boxY + 29,
      { width: 340, ellipsis: true },
    );
  doc.font("Helvetica").fontSize(9).fillColor(COLOR_MUTED)
  ...
  if (isFreeDdt) {
    doc.font("Helvetica").fontSize(8).fillColor(COLOR_MUTED)
      .text(`DDT libero ${formatDate(ddt.createdAt || order?.createdAt)}`, MARGIN + 360, boxY + 34);
...
function drawItemsSection(doc, order) {
  ...
  const descW = CONTENT_W - 180;
  const qtyW = 36;
  const umW = 34;
  const noteW = 84;
  const qtyX = MARGIN + 10 + descW + 5;
  const umX = qtyX + qtyW + 5;
  const noteX = umX + umW + 5;

  doc.font("Helvetica-Bold").fontSize(9).fillColor(COLOR_MUTED)
    .text("ARTICOLI TRASPORTATI", MARGIN + 10, startY + 8)
    .text("QTA", qtyX, startY + 8, { width: qtyW, align: "right" })
    .text("UM", umX, startY + 8, { width: umW })
    .text("NOTE", noteX, startY + 8, { width: noteW });
  ...
  lines.forEach((item) => {
    ...
    doc.font("Helvetica").fontSize(9.5).fillColor(COLOR_TEXT)
      .text(String(item.title || "Prodotto"), MARGIN + 10, titleY, { width: descW });
    const afterTitleY = doc.y;
    doc.font("Helvetica").fontSize(9.5)
      .text(String(item.quantity || 1), qtyX, titleY, { width: qtyW, align: "right" });
    if (item.um) {
      doc.font("Helvetica").fontSize(9.5).fillColor(COLOR_TEXT)
        .text(String(item.um), umX, titleY, { width: umW });
    }
    if (item.note) {
      doc.font("Helvetica").fontSize(8.5).fillColor(COLOR_MUTED)
        .text(String(item.note), noteX, titleY, { width: noteW });
    }
...
function drawFooter(doc, order) {
  ...
  const ref = order?.__ddtFree
    ? `DDT libero ${order?.operations?.warehouse?.ddt?.number || order?.orderNumber || ""}`.trim()
    : `Ordine ${order?.orderNumber || ""}`.trim();
  doc.font("Helvetica").fontSize(7).fillColor(COLOR_MUTED)
    .text(
      `Documento generato da Prato Sintetico Italia — ${ref}`,
